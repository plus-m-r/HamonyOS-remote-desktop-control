...
/// 增强版ConnectionService - 包含真实WebSocket实现
/// 
/// 本服务负责与Java服务器建立WebSocket连接并管理：
/// - 实时屏幕帧接收
/// - 鼠标/键盘事件发送
/// - 连接状态管理
/// - 自动重连机制
/// - 心跳保活
class ConnectionServiceEnhanced extends GetxService {
  final logger = Logger();
  ...
  /// 打开远程屏幕
  Future<bool> openRemoteScreen(String deviceCode, String password) async {
    try {
      isConnecting.value = true;
      ...
      _wsChannel?.sink.add(jsonEncode({
        'type': 'openRemoteScreen',
        'deviceCode': deviceCode,
        'password': password,
      }));

      isConnecting.value = false;
      logger.i('远程屏幕打开命令已发送');
      return true;
    } catch (e) {
      logger.e('打开远程屏幕失败: $e');
      isConnecting.value = false;
  ...
  Future<void> closeRemoteScreen() async {
    try {
      ...
      _wsChannel?.sink.add(jsonEncode({
        'type': 'closeRemoteScreen',
      }));
      
      logger.i('远程屏幕关闭命令已发送');
    } catch (e) {
      logger.e('关闭远程屏幕失败: $e');
      rethrow;
    }
  ...
  void _handleConnectionError() {
    ...
    final newStatus = connectionStatus.value.copyWith(
      ...
      errorMessage: '连接失败，正在重新连接... ($_reconnectAttempts/$maxReconnectAttempts)',
    );
    connectionStatus.value = newStatus;
    _statusStreamController.add(newStatus);
