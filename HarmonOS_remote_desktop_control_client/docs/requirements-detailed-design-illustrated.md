...
本章详细设计以实际 ArkTS 源码为基础，按页面设计、数据设计、关键技术与难点三个大部分展开，并为每个重点部分补写图示说明。

## 1. 界面设计

HarmonyOS 客户端采用 MVVM 架构，页面层只负责渲染与事件转发，业务逻辑由 ViewModel 和服务层承担。页面组件与业务服务之间通过依赖注入解耦，界面层保持轻量。

### 1.1 典型使用流程

- 启动应用后进入首页（`entry/src/main/ets/pages/Index.ets`），用户输入设备 ID、密码，并可复制设备信息。
- 首页通过 `IndexViewModel` 调用 `remoteControlService.validateDevice` 验证设备信息，验证成功后调用 `remoteControlService.setDeviceInfo`，并通过路由导航进入控制页面。
- 控制页面（`entry/src/main/ets/pages/Control.ets`）初始化 `ControlViewModel`，通过 `ServiceProvider` 注入 `remoteControlService`、`fileService`、`clipboardService` 等依赖。
- `ControlViewModel` 设置连接状态回调和屏幕数据回调，并在设备凭据准备好后调用 `remoteControlService.openRemoteScreen` 发起远程屏幕捕获。
- 远程屏幕数据在 `TcpClient` 收到后交给 `ProtocolHandler` 解析，再由 `RemoteControlService` 进入 `CaptureProcessor` / `CaptureCache` 处理，最终由 `Control.ets` 的 `Image(PixelMap)` 显示。
- 用户在远程屏幕页进行触控、拖拽、缩放、键盘输入或三指手势，`ControlViewModel` 将这些事件转为控制命令，并调用远程控制服务发送到服务器。
- 用户可通过设置页面（`entry/src/main/ets/pages/Settings.ets`）调整网络参数、同步策略、文件传输和剪贴板配置，保存后由 `AppStateManager` 持久化。

> 图 1：典型页面导航与操作流程图

图示说明：该图展示 `Index -> Control -> Settings` 的典型用户路径，标注首页输入、验证、远程屏幕初始化、画面显示、交互事件转发、设置保存以及后台服务调用关系。
...
图示说明：该图说明用户在设置页面修改参数后，如何流转至 `AppStateManager`、`ConfigCenter` 与各服务模块。它还标注了设置页面与远程控制、剪贴板、文件传输模块之间的配置依赖关系。
...
图示说明：该图描述 `ConfigCenter`、`AppStateManager`、`IStateStorage` 之间的层次关系，展示配置读取、状态缓存与持久化保存的职责分工，以及页面层与服务层对它们的访问方式。
...
图示说明：该图展示通信链路、协议解析、帧处理、服务与 UI 之间的调用关系，突出 `TcpClient`、`RemoteControlService`、`CaptureProcessor` 与 `ControlViewModel` 的协同过程，并说明异常重连与状态回调如何传导。
...
该设计建议在后续评审中补充四张图：页面流程图、设置与状态管理关系图、本地配置与缓存层次图、关键技术架构图，以便更直观地说明客户端架构。
